...
public class IslandScenario extends Scenario
{
	...
	public void init()
	{
		...
		mPlayer.setTilePosition(Constants.PLAYER_INITIAL_POSITION, Constants.PLAYER_INITIAL_POSITION);
	}
	...
	private PathOfMoving findPath(Point fromPoint)
	{
		...
		while (!q.isEmpty())
		{
			...
			for (int[] m : moves)
			{
				...
				if (inside(map, next) && !visited[next.y][next.x] && mMap.getType(next.y, next.x) != MapImage.BLOCKED)
				{
