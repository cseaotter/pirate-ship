...
public class BattleField extends Scenario
{
    ...
    private void init()
    {
        try
        {
            ...
            mBullet = ImageIO
                .read( getClass().getResourceAsStream( Constants.RESOURCE_CANNON_BALL ) );

            BufferedImage bi = ImageIO
                .read( getClass().getResourceAsStream( Constants.RESOURCE_EXPLOSION ) );
    ...
    public void draw( Graphics2D g2 )
    {
        ...
        g2.drawImage( mCastleWall,
            Constants.WIDTH - mCastleWall.getWidth() + 200,
            Constants.HEIGHT - mCastleWall.getHeight() - mWater.getHeight(),
            null );
        ...
        if ( explosion != 8 || shoot )
        {
            if ( role == Constants.ROLE_PIRATE && y >= 200 && y < 800
                && x >= Constants.WIDTH - mCastleWall.getWidth() + 200
                || hitPirate( (int)x, (int)y ) )
            {
    ...
    private void drawRemoteBullet( Graphics2D g2 )
    {
        if ( remoteWeapon != null && remoteWeapon.isBulletActive() )
        {
            ...
            {
                g2.drawImage( mExplosion[remoteWeapon.getExplosionFrame()],
                    bulletLoc.x,
                    bulletLoc.y,
                    null );
    ...
    private boolean hitPirate( int x, int y )
    {
        ...
        return y >= point.y && y <= point.y + mBoat.getHeight() && x >= point.x
            && x <= point.x + mBoat.getWidth();
    ...
    private void drawCannonOnCastle( Graphics2D g2 )
    {
        ...
        g2.drawImage( op.filter( image, null ),
            Constants.WIDTH - mCastleWall.getWidth() + 250,
            500,
            null );
    }
    ...
    private void drawHealthBar( Graphics2D g2 )
    {
        ...
        if ( king != null )
        {
            ...
            g2.fill3DRect( Constants.WIDTH - mCastleWall.getWidth() + 200,
                Constants.HEIGHT - mCastleWall.getHeight() - mWater.getHeight() - 100,
                king.getHealth() * 300 / Constants.KING_HEALTH,
                50,
                true );
        }
